...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens, seq_length = pad_packed_sequence(sequence=enc_hiddens,batch_first=True)    #pack tensor containing padded sequences of variable length
        init_decoder_hidden = torch.cat((last_hidden[0], last_hidden[1]), 1)    #flatten hidden state 3D -> 2D
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     Tensor Squeeze:
        ###         https://pytorch.org/docs/stable/torch.html#torch.squeeze
        dec_state = self.decoder(Ybar_t, dec_state)             #WHERE IS THIS FUNCTION????
        dec_hidden, dec_cell = dec_state[0], dec_state[1]
        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden,2))        #(b, src_len, h) x (b, h, 1) -> (b, src_len, 1)
        e_t = torch.squeeze(e_t, dim=-1)                                        #(b, src_len, 1) -> (b, src_len)
        ### END YOUR CODE
